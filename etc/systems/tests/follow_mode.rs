...
static FOLLOW_MODE_TEST_LOCK: tokio::sync::Mutex<()> = tokio::sync::Mutex::const_new(());

#[tokio::test]
async fn follow_mode_missing_requests_hash_reproduces_withdrawals_root_bug() -> Result<()> {
    let _guard = FOLLOW_MODE_TEST_LOCK.lock().await;
    ...
    let system = SystemTestStackBuilder::new()
        ...
        .with_stripped_follow_source_requests_hash()
        .build()
        .await?;
