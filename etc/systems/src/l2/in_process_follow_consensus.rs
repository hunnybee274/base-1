...
impl InProcessFollowConsensus {
    ...
    pub async fn start(config: InProcessFollowConsensusConfig) -> Result<Self> {
        ...
        let engine_client = Arc::new(
            engine_config
                .build_engine_client()
                .await
                .map_err(|e| eyre::eyre!("failed to build follow engine client: {e}"))?,
        );
        ...
        let handle = tokio::spawn(async move {
            ...
            if let Err(e) = node.start().await {
                let _ = startup_tx.send(Err(eyre::eyre!("follow consensus node failed: {e}")));
                return;
            }
        ...
        tokio::select! {
            result = startup_rx => {
                result.wrap_err("startup channel closed")?
                      .wrap_err("follow consensus node failed during startup")?;
            }
            result = Self::wait_for_rpc(rpc_addr) => {
                result?;
            }
        }
    ...
    /// Waits until the follow consensus RPC endpoint is ready to accept requests.
    pub async fn wait_for_rpc(addr: SocketAddr) -> Result<()> {
        ...
        for i in 0..60 {
            match client.get(&url).send().await {
                Ok(_) => {
                    info!(attempts = i + 1, "follow consensus RPC is ready");
                    return Ok(());
                }
                Err(e) => {
                    last_err = Some(e);
        ...
        Err(eyre::eyre!(
            "Follow consensus RPC at {url} did not become ready within 30s: {}",
            last_err.unwrap()
        ))
    }
}
