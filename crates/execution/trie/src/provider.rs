...
pub struct BaseProofsStateProviderRef<'a, Storage: BaseProofsStore> {
    ...
    /// so that all EVM state reads within a single execution context share one MDBX snapshot
    /// and avoid per-call `lck_rdt_lock` contention.
...
impl<'a, Storage: BaseProofsStore + Clone> BaseProofsStateProviderRef<'a, Storage> {
    fn storage_by_hashed_key(
        ...
        let tx = guard.as_ref().expect("just initialized");
        Ok(self
            .storage
